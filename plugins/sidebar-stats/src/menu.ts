	...
	return {
		...
		async saveDisplayDefaults(): Promise<void> {
			...
			try {
				...
				if (!isActive()) return;
				notify("Sidebar Stats display defaults saved", "info");
			} catch (error) {
				if (!isActive()) return;
				notify(
		...
		const hostPromise = ctx.ui.custom<void>(
			(tui, theme, _keys, done) => {
				...
				const ensureActive = (): void => {
					if (!isOverlayLifetimeActive(lifetime)) throw new Error("Sidebar Stats is not active in this session");
				};
				if (!isOverlayLifetimeActive(lifetime)) finish();
				const component = createLifecycleOverlayComponent(
					lifetime,
...
export async function openSidebarControlCenter(
	...
	const lifetime = options.lifetime;
	if (ctx.mode !== "tui") {
		ctx.ui.notify("Sidebar Stats Control Center requires TUI mode", "warning");
		return;
	}
	if (!isOverlayLifetimeActive(lifetime)) return;
